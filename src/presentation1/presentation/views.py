...
def mainpage_view(page: ft.Page):
    ...
    profile = ft.Text(page.session["login_input"], size=22)
    a = ft.Row(
        [
...
def settings_view(page: ft.Page):
    page.session.set("old_pass", ft.TextField(label="Старый пароль", width=300))
    page.session.set("old_pass", ft.TextField(label="Новый пароль", password=True, width=300))
...
def change_password_error(type_err, page: ft.Page):
    if type_err == 1:
        ...
        error_text = ft.Text("Неверный пароль.",
                             size=18, weight=ft.FontWeight.NORMAL, color="red", text_align=ft.TextAlign.CENTER)
    page.session.set("old_pass", ft.TextField(label="Старый пароль", width=300, border_color="red"))
    page.session.set("old_pass", ft.TextField(label="Новый пароль", password=True, width=300, border_color="red"))
    exit_button = ft.ElevatedButton(
        content=ft.Text("Выйти из профиля", size=20, color=ft.colors.RED),
        bgcolor=ft.colors.DEEP_ORANGE_100,
        color=ft.colors.WHITE,
        ...
        topic_row = ft.Row(
            controls=[
                ...
                ft.ElevatedButton(content=ft.Text("Удалить", size=18, color=ft.colors.WHITE),
                                  bgcolor=ft.colors.BLUE_ACCENT_700, on_click=lambda _: cl.delete_click(topic["name"]))
            ],
            alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
            vertical_alignment=ft.alignment.top_center,
        )
